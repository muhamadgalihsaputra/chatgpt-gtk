...
def is_same_app_domain(url_str):
    ...
    try:
        ...
        return any(d in host for d in ("chatgpt.com", "openai.com", "oaistatic.com", "oaiusercontent.com"))
    except Exception:
        return False
...
class ChatGPTWindow(Adw.ApplicationWindow):
    ...
    def send_desktop_notification(self, summary, body, urgency=2):
        ...
        try:
            ...
            hints = {
                "desktop-entry": GLib.Variant("s", "chatgpt-gtk"),
            ...
            print(f"[chatgpt-gtk] Notifikasi desktop terkirim: {summary} - {body}")
        except Exception as e:
            print(f"[chatgpt-gtk] D-Bus notification error: {e}")

        try:
            notif = Gio.Notification.new(summary)
            notif.set_body(body)
            notif.set_priority(Gio.NotificationPriority.HIGH)
            self.app.send_notification("chatgpt-notify", notif)
        except Exception:
            pass
    ...
    def on_show_notification(self, web_view, notification):
        title = notification.get_title() or "ChatGPT"
    ...
    def on_generation_done(self, manager, js_result):
        print("[chatgpt-gtk] Selesai generate respon!")
        if not self.is_active() or not self.get_visible():
            ...
            if hasattr(self, "tray") and self.tray:
                self.tray.set_attention(True, "ChatGPT: Jawaban selesai dibuat")
        ...
        if uri and uri not in ("about:blank", ""):
            if not is_auth_url(uri) and not is_same_app_domain(uri):
                try:
                    ...
                    print(f"[chatgpt-gtk] Error launching default browser: {e}")
                return None
        ...
        def on_decide_destination(dl, suggested_filename):
            try:
                ...
                def on_finished(d):
                    ...
                    print(f"[chatgpt-gtk] Download selesai: {dest_path}")
                    self.send_desktop_notification(
