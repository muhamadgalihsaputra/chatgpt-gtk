...
class ChatGPTWindow(Adw.ApplicationWindow):
    ...
    def on_show_notification(self, web_view, notification):
        ...
        if body:
            notif.set_body(body)
        notif.set_priority(Gio.NotificationPriority.HIGH)
        self.app.send_notification("chatgpt-web-notif", notif)
        return True
    ...
    def on_generation_done(self, manager, js_result):
        if not self.get_visible() or not self.is_active():
            ...
            notif.set_priority(Gio.NotificationPriority.HIGH)
            self.app.send_notification("chatgpt-generation-done", notif)
        ...
        def on_decide_destination(dl, suggested_filename):
            try:
                ...
                def on_finished(d):
                    ...
                    notif.set_priority(Gio.NotificationPriority.HIGH)
                    self.app.send_notification("chatgpt-download-finished", notif)
